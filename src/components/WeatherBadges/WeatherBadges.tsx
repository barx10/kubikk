...
export function WeatherBadges({ points, loading }: WeatherBadgesProps) {
  if (loading) {
    return <p className="text-sm text-slate-500">Henter værvarsel for ruten…</p>;
  }
  if (points.length === 0) {
    return null;
  }
  ...
  return (
    <div className="flex gap-2 overflow-x-auto pb-1">
      {points.map((wp, i) => (
        <div
          key={i}
          className="flex shrink-0 flex-col items-center rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs shadow-sm"
        >
          <span className="text-slate-500">{wp.point.name ?? `Punkt ${i + 1}`}</span>
          <span className="text-base font-semibold text-slate-900">
            {wp.temperatureC !== null ? `${Math.round(wp.temperatureC)}°C` : "–"}
          </span>
          {wp.precipitationMm !== null && wp.precipitationMm > 0 && (
            <span className="text-blue-600">{wp.precipitationMm} mm</span>
          )}
        </div>
      ))}
    </div>
